...
app.controller('InventarioCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster','$timeout',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster,$timeout) {
 $scope.nivel=MyService.data.nivel;
  ...
  $scope.carga = function (){
    $http.get('http://192.168.1.100:1337/categoria/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
...
MyService.data.luz=null; 
    ...
    $http.get('http://192.168.1.100:1337/articulo/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
...
// };
  ...
  $http.get('http://192.168.1.100:1337/departamento/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
...
$scope.openPeso = function (item) {
  ...
  $scope.deleteCategoria = function(item){
    $http.delete('http://192.168.1.100:1337/categoria/'+item.id , item)
    $scope.categorias.splice($scope.categorias.indexOf(item), 1);
  };
  ...
  $scope.calculoExistencia=function(idArticulo){
    ...
    $http.get('http://192.168.1.100:1337/entrada/?idArticulo='+idArticulo).then(function (resp) {
      $scope.entradas=resp.data.results;
      for (var i=0;i<$scope.entradas.length;i++){
        entradas=entradas+$scope.entradas[i].cantidad;
        }
        $http.get('http://192.168.1.100:1337/salida/?idArticulo='+idArticulo).then(function (resp) {
  ...
  $scope.selectItem = function(item){    
    ...
    //  $http.get('http://192.168.1.100:1337/peso/?idArticulo='+identificador).then(function (resp) {
  ...
  $scope.deleteItem = function(item){
    $http.delete('http://192.168.1.100:1337/articulo/'+item.id , item)
  ...
  $scope.deleteArticulo = function(articulo){
    $http.delete('http://192.168.1.100:1337/articulo/'+articulo.id , articulo)
  ...
  $scope.doneEditingCategoria = function(item){
    ...
    if (MyService.data.idenCategoria){
      $http.put('http://192.168.1.100:1337/categoria/'+MyService.data.idenCategoria, categoriaAct)
    }
    else {
      $http.post('http://192.168.1.100:1337/categoria/', categoriaAct)
    }
    // $http.get('http://192.168.1.100:1337/categoria/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
    //   $scope.categorias = resp.data.results;
    // });
  ...
  articuloAct.estado=item.estado;
   articuloAct.control=false;
    ...
    if (MyService.data.idenArticulo){
      $scope.pop4();
      $http.put('http://192.168.1.100:1337/articulo/'+MyService.data.idenArticulo , articuloAct)
    }
    else {
      $scope.pop3();;
      $http.post('http://192.168.1.100:1337/articulo/', articuloAct)
    }
    $http.get('http://192.168.1.100:1337/categoria/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
      $scope.categorias = resp.data.results;
    });
