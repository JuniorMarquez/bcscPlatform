...
app.controller('ContactCtrl', ['$scope', '$http', '$filter', '$modal', 'MyService', 'filterFilter', function($scope, $http, $filter,$modal, MyService,filterFilter) {
  ...
  $scope.filter = '';
   ...
   $http.get('http://192.168.1.100:1339/genero').then(function (resp) {
...
// alert("servicio: " +MyService.data.identificador);
 // $http.get('http://192.168.1.100:1339/cancion/?idDisco='+MyService.data.identificador).then(function (resp) {
 ...
 //  });
    ...
    //   $http.post('http://192.168.1.100:1339/api/cancion/' ,item);
 ...
 $scope.disabled = undefined;
        ...
        // return $http.get(
        //   'http://192.168.1.100:1339/api/genero'
  ...
  $scope.deleteGroup = function(item){
     ...
     $http.delete('http://192.168.1.100:1339/genero/'+item._id , item)
...
$http.get('http://192.168.1.100:1339/disco').then(function (resp) {
 ...
 document.getElementById('panel').style.visibility='hidden';
  ...
  $scope.selectItem = function(item){    
    ...
    // alert("id:" +item._id);
      ...
      $http.get('http://192.168.1.100:1339/cancion').then(function (resp) {
    $scope.canciones = resp.data;
    });
  ...
  $scope.deleteItem = function(item){
    $http.delete('http://192.168.1.100:1339/disco/'+item._id , item)
...
$scope.deleteCancion = function(cancion){
    $http.delete('http://192.168.1.100:1339/cancion/'+cancion._id , cancion)
...
//  
  if (cancion.id){
     $http.put('http://192.168.1.100:1339/cancion/'+cancion.id , cancion)
   }
     else {
  $http.post('http://192.168.1.100:1339/cancion/' , cancion);
   }
   $scope.items = null;
    $scope.cancion = null;
 ...
 //     $http.put('http://192.168.1.100:1339/genero/'+MyService.data.idenGen, MyService.data.actualizado);
...
if (MyService.data.idenGen){
     $http.put('http://192.168.1.100:1339/genero/'+MyService.data.idenGen, generoAct)
}
else {
 $http.post('http://192.168.1.100:1339/genero/', generoAct)
    }
 $http.get('http://192.168.1.100:1339/genero').then(function (resp) {
    $scope.groups = resp.data;
 });
  ...
  if (MyService.data.idenDisc){
     $http.put('http://192.168.1.100:1339/disco/'+MyService.data.idenDisc , discoAct)
   }
  else {
    $http.post('http://192.168.1.100:1339/disco/', discoAct)

 // $http.get('http://192.168.1.100:1339/api/disco').then(function (resp) {
